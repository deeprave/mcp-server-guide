...
def test_category_add_help():
    """Test that 'category add --help' is detected as help request."""
    result = parse_command(["category", "add", "--help"])
    assert result.type == "help", f"Expected help, got {result.type}"
    assert result.target == "category", f"Expected target 'category', got {result.target}"


def test_collection_add_help():
    """Test that 'collection add --help' is detected as help request."""
...
def test_document_create_help():
    """Test that 'document create --help' is detected as help request."""
