...
class TestCommandHandlerErrors:
    ...
    async def test_unknown_command_error(self, handler):
        ...
        result = await handler.execute_command("unknown", [], {}, "subcommand")
    ...
    async def test_category_command_error(self, handler):
        """Test category command error handling."""
        with patch.object(handler, "_execute_category_command") as mock_category:
            mock_category.return_value = {"success": False, "error": "Category not found"}

            result = await handler.execute_command("guide", [], {}, "nonexistent")
    ...
    async def test_guide_new_success_path(self, handler):
        ...
        with patch.object(handler, "_execute_guide_new_command") as mock_new:
            ...
            result = await handler.execute_command("guide", ["test"], {"dir": "/test"}, "new")
    ...
    async def test_category_shortcut_execution(self, handler):
        ...
        with patch.object(handler, "_execute_category_command") as mock_category:
            mock_category.return_value = {"success": True, "content": "Category content"}

            result = await handler.execute_command("guide", [], {}, "python")

            assert result["success"] is True
            mock_category.assert_called_once_with("python", [])
