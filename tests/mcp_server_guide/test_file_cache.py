...
async def test_cache_entry_needs_validation():
...
async def test_http_client_conditional_requests():
...
async def test_http_client_conditional_with_etag():
...
async def test_http_client_conditional_modified():
...
async def test_file_cache_with_validation():
...
async def test_file_accessor_with_http_caching():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with patch("mcp_server_guide.http.async_client.AsyncHTTPClient") as mock_client_class:
            ...
            content1 = await accessor.read_file("guide.md", http_source)
            ...
            with patch("time.time", return_value=time.time() + 400):  # 400 seconds later
                ...
                content2 = await accessor.read_file("guide.md", http_source)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with patch("mcp_server_guide.http.async_client.AsyncHTTPClient") as mock_client_class:
            ...
            with patch("time.time", return_value=time.time() + 400):  # 400 seconds later
                ...
                content3 = await accessor.read_file("guide.md", http_source)
                assert content3 == "# Guide v2"


async def test_cache_respects_cache_control():
