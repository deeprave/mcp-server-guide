...
async def test_list_resources_returns_empty_list_when_no_auto_load_true_categories():
...
async def test_read_resource_serves_correct_content_for_valid_uris():
    ...
    with patch("mcp_server_guide.server.tools.get_category_content") as mock_get_content:
        mock_get_content.return_value = AsyncMock(
            return_value={"success": True, "content": "# Test Content\n\nThis is test content."}
        )
        mock_get_content.side_effect = lambda *args, **kwargs: AsyncMock(
            return_value={"success": True, "content": "# Test Content\n\nThis is test content."}
        )()

        # Simpler approach - make it return the dict directly since we're awaiting it
        async def mock_return(*args, **kwargs):
            return {"success": True, "content": "# Test Content\n\nThis is test content."}

        mock_get_content.side_effect = mock_return

        content = await read_resource("guide://test_category")

        assert content == "# Test Content\n\nThis is test content."
        mock_get_content.assert_called_once_with("test_category", None)


async def test_read_resource_raises_error_for_invalid_uris():
...
async def test_read_resource_handles_missing_categories_gracefully():
    ...
    with patch("mcp_server_guide.server.tools.get_category_content") as mock_get_content:
        ...
        try:
            await read_resource("guide://nonexistent")
...
async def test_mcp_server_exposes_resource_endpoints():
