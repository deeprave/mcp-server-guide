...
@pytest.mark.asyncio
async def test_execute_g_colon_category():
    """Test executing /g:category command."""
    handler = CommandHandler()

    with patch("mcp_server_guide.commands.handler.get_category_content") as mock_get_category:
        mock_get_category.return_value = {"success": True, "content": "lang content"}

        result = await handler.execute_command("g", [], {}, "lang")
...
@pytest.mark.asyncio
async def test_execute_g_colon_edit():
    """Test executing /g:edit command."""
    handler = CommandHandler()
    ...
    with (
        ...
        params = {"patterns": ["*.ts", "*.tsx", "*.d.ts"]}
        result = await handler.execute_command("g", ["typescript"], params, "edit")
...
@pytest.mark.asyncio
async def test_execute_g_help():
    """Test executing /g:help command."""
    handler = CommandHandler()
