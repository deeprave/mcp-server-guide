...
async def test_get_all_guides_returns_empty_dict_when_no_auto_load_true_categories():
    ...
    with patch("mcp_server_guide.tools.content_tools.SessionManager") as mock_session_class:
        ...
        config_data = ProjectConfig(
            categories={
                "guide": Category(dir="guide/", patterns=["*.md"], description="Guide", auto_load=False),
                "lang": Category(dir="lang/", patterns=["*.md"], description="Language", auto_load=False),
            }
        )
        mock_session.get_or_create_project_config = AsyncMock(return_value=config_data)
...
@pytest.mark.asyncio
async def test_get_all_guides_empty_config():
...
@pytest.mark.asyncio
async def test_get_all_guides_no_categories_key():
...
@pytest.mark.asyncio
async def test_get_all_guides_exception_in_get_or_create_project_config():
    """Test handling when get_or_create_project_config raises an exception."""
    with patch("mcp_server_guide.tools.content_tools.SessionManager") as mock_session_class:
        mock_session = Mock()
        mock_session_class.return_value = mock_session
        mock_session.get_project_name.return_value = "test-project"  # NOT async
