...
from mcpguide.server import create_server
from mcpguide.session_tools import SessionManager


def test_server_uses_hybrid_file_access():
...
def test_server_resolves_http_resources():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(server, '_session_manager', session):
            with patch('mcpguide.http_client.HttpClient') as mock_client_class:
                ...
                content = server.read_guide("test-project")
...
def test_server_caches_http_resources():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(server, '_session_manager', session):
            with patch('mcpguide.http_client.HttpClient') as mock_client_class:
                ...
                content1 = server.read_guide("test-project")
                ...
                with patch.object(server.file_accessor.cache, 'get') as mock_cache_get:
                    ...
                    content2 = server.read_guide("test-project")
                    assert content2 == "# Cached Guide"
                    assert mock_client.get_conditional.call_count == 1


def test_server_handles_mixed_sources():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(server, '_session_manager', session):
            with patch('mcpguide.http_client.HttpClient') as mock_client_class:
                ...
                guide_content = server.read_guide("mixed-project")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(server, '_session_manager', session):
            with patch('mcpguide.http_client.HttpClient') as mock_client_class:
                ...
                content1 = server.read_guide("fallback-project")
                assert content1 == "# Cached Guide"
...
def test_server_integration_with_session_paths():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(server, '_session_manager', session):
            ...
            lang_source = server._get_file_source("language", "integration-project")
            assert lang_source.type == "server"

            context_source = server._get_file_source("context", "integration-project")
            assert context_source.type == "http"
            assert context_source.base_path == "https://example.com/context.md"


def test_server_respects_cache_settings():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(server, '_session_manager', session):
            with patch('mcpguide.http_client.HttpClient') as mock_client_class:
                ...
                source = server._get_file_source("guide", "cache-test")
                assert source.cache_enabled is True

                # Read should cache
                content = server.read_guide("cache-test")
