...
def mock_server():
    ...
    def mock_resource_decorator(uri, name=None, description=None, mime_type=None):
        def decorator(func):
            server._resources[uri] = {"func": func, "name": name, "description": description, "mime_type": mime_type}
            return func
...
def mock_config():
    ...
    config = MagicMock()
    config.categories = {
        "test_cat": Category(name="test_cat", dir="/test", patterns=["*.py"], description="Test category")
    }
    config.collections = {
        "test_coll": Collection(name="test_coll", categories=["test_cat"], description="Test collection")
    }
    return config
...
async def test_register_resources(mock_server, mock_config):
    ...
    with (
        patch("mcp_server_guide.resource_registry._register_category_resources", new_callable=AsyncMock) as mock_cat,
...
async def test_register_category_resources(mock_server, mock_config):
    ...
    with (
        patch("mcp_server_guide.resource_registry.get_category_content", new_callable=AsyncMock) as mock_get_cat,
