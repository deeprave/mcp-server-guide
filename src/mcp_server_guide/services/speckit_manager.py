...
async def is_speckit_enabled() -> bool:
    ...
    return await session.is_speckit_enabled()


async def get_speckit_config() -> SpecKitConfig:
    ...
    """
    session = SessionManager()
    config = await session.get_speckit_config()
    if config is not None:
        return config
    return SpecKitConfig(enabled=False, url="https://github.com/spec-kit/spec-kit", version="latest")


async def enable_speckit(url: str = "https://github.com/spec-kit/spec-kit", version: str = "latest") -> None:
    ...
    await session.set_speckit_config(config)


async def update_speckit_config(updates: Dict[str, Any]) -> None:
    ...
    """
    session = SessionManager()
    current_config = await session.get_speckit_config()
    ...
    await session.set_speckit_config(updated_config)
